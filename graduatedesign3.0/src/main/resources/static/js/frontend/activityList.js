$(function() {
	...
	//获取标签列表的URL，初始化为1
	var pageNum = 1;
	var activityCategoryId = getQueryString('activityCategoryId');
	var searchDivUrl = '/frontend/activityCategory/getActivityTags';
	var activitystatus = '';
	var tagId = '';
	var keyword = '';

	function getSearchDivData() {
		//如果传入了活动类别id，则取出活动类别下的所有活动标签（二级类别）
			...
			if (data.success) {
				...
				data.activityList.map(function(item, index) {
					html += '' + '<div class="card" data-shop-id="'
							...
							+ '更新</p>' +'<span>'+item.status+'</span>'+'<span>点击查看</span>' + '</div>'
							+ '</div>';
				});
				//将卡片集合添加到目标HTML组件里
	...
	$('.shop-list').on('click', '.card', function(e) {
		...
		window.location.href = '/myo2o/frontend/shopdetail?shopId=' + shopId; //获取活动详情URL
	});
	...
	$('#shoplist-search-div').on(
		...
		//		}
		// 		else {
		// 			// 如果传递过来的活动类别为空，则按查询全部标签下的活动
		// 			activityCategoryId = e.target.dataset.categoryId;
		// 			if ($(e.target).hasClass('button-fill')) {
		// 				$(e.target).removeClass('button-fill');
		// 				activityCategoryId = '';
		// 			} else {
